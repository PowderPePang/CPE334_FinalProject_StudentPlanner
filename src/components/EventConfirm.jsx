            ...
            <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '2rem', marginTop: '2rem' }}>
              <div>
                <div style={{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '12px', border: '2px solid #e0e0e0', textAlign: 'center' }}>
                  {qrCodeUrl && (<img src={qrCodeUrl} alt="QR Code" style={{ width: '200px', height: '200px', margin: '0 auto' }} />)}
                  <p style={{ marginTop: '1rem', color: '#666', fontSize: '0.9rem' }}>Show this QR Code at the event entrance</p>
                </div>
              </div>

              <div>
                <h1 style={{ fontSize: '2rem', fontWeight: 'bold', color: '#333', marginBottom: '1.5rem' }}>{event?.title}</h1>

                <div style={{ backgroundColor: '#f8f9fa', padding: '1.5rem', borderRadius: '8px', marginBottom: '1.5rem' }}>
                   <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}><Calendar size={18} color="#666" /><span style={{ color: '#333', fontWeight: '500' }}>{event?.date || event?.startDate || 'TBA'}</span></div>
